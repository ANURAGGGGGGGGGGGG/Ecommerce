...
import { getProductById, updateProduct } from "@/data/product";
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        const productToEdit = await getProductById(idStr);
        if (!productToEdit) {
          setNotFound(true);
        } else {
          if (productToEdit.sellerId && productToEdit.sellerId !== parsed.email) {
            setNotFound(true); // Prevent editing other seller's products
            return;
          }
          ...
          setForm({
            ...
            stock: (productToEdit.stock ?? 10).toString(),
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!validate() || !user || notFound) return;
    setLoading(true);
    
    try {
      const ok = await updateProduct(idStr, {
        name: form.name.trim(),
      ...
      if (ok) {
        ...
        setErrors((prev) => ({ ...prev, name: "Failed to save changes." }));
      }
    } catch (err) {
  ...
  if (notFound) {
    return (
      <div style={{ minHeight: "100vh", background: "#13121b", color: "#e4e1ee", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
        <h2>Product Not Found</h2>
        <Link href="/seller/products" style={{ color: "#8781ff", marginTop: 16 }}>Return to Products</Link>
      </div>
    );
  }
      ...
      <main className="mobile-p-sm" style={{ flex: 1, padding: "40px", overflow: "auto" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ margin: "8px 0 0", color: "#918fa1", fontSize: 15 }}>
            Update your product details and save changes.
          </p>
        </div>
